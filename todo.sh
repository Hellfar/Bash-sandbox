...
do
	...
	do
		...
		export descr=\`echo "$line" | sed -e "s/^[$UDONEC$DONEC]\\s//"\`
	...
	echo "$params" | xargs $DIALOG --backtitle "TODO LIST SOFTWARE" \
		--title "TODO" --clear \
			--checklist "Here is your current tasks (`expr 100 \* \`grep '^\*' TODO | wc -l\` / \`cat TODO | wc -l\``% done):" `expr $lines - 5` $cols $entries \
	...
	#ticking the selected ones.
				echo "$choices" | xargs -I{} bash -c "sed -ie '{}s/^$UDONEC/$DONEC/' $TODO"
	#unticking those are not selected.
				cat <(nl $TODO | grep -oP "^\s*\d+" | tr -d ' ') <(echo "$choices") | sort | uniq -u | xargs -I{} bash -c "sed -ie '{}s/^$DONEC/$UDONEC/' $TODO"
			fi
